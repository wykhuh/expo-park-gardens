...
create_year_chart <- function(df) {
  ggplot(data=df, mapping=aes(x=year, y=n, color=place)) + 
...
I downloaded iNaturalist and eBird (Cornell Lab of Ornithology) data from 2014 to 2023 at Natural History Museum of Los Angeles County and Rose Garden using GBIF.org. GBIF only includes research grade iNaturalist observations. I chose the end of 2023 as the cutoff date because GBIF does not currently have eBird records after December 2023. I also filtered the iNaturalist observations to have coordinate uncertainty of 100 meters or less. eBird observations do not provide coordinate uncertainty data. I filtered the iNaturalist observations where taxon rank is 'FORM', 'SPECIES', 'SUBSPECIES' or 'VARIETY'. eBird only has 'SPECIES'. Here's the code to [clean the data](clean_gbif.qmd)
...
Comparison of the number of iNaturalist and eBird observations. NHM has more eBird observations than iNaturalist observations. Rose Garden has more iNaturalist observations than eBird observations. It is surprising to see so few eBird observations at the Rose Garden. It is as if birders are purposefully avoiding the Rose Garden.
...
gbif_nhm_platform_df <- gbif_nhm_df |>
  count(year, institutionCode)
  
ggplot(data=gbif_nhm_platform_df, 
       mapping=aes(x=year, y=n, color=institutionCode)) + 
  geom_line(  ) +
  theme_bw()
```
...
Top ten most observed species at NHM. All birds since eBird has more observations than than iNaturalist at NHM.
...
Top ten most observed species at Rose Garden. Wider variety than NHM since iNaturalist has more observations than eBird.
